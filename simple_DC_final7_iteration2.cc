...
Double_t Sterile ::FillEv( RooListProxy* _pulls ) const 
{
   ...
   *///June,3,2020 : Print out Scaling4
   TVectorD* fVec = new TVectorD(nBins * 5);//Up to date prediction. current prediction
...
*/
   ...
   if (singleExp.Contains("NEOS")){
     ...
     for(Int_t i=0;i<nBins;i++){
        ...
        SumfVec += (*fVec)[3 * nBins + i] * scaling4;//May,20,2020
  
        (*testVec)[i]=(*fVec)[3 * nBins + i] * scaling4; //May,21,2020
   ...
   */
   ...
   TMatrixD* covMat = this->prepareCovMatrix(nBins , fVec);// fVec=Up to date prediction, current prediction 
    /*
...
std::vector<TH1D*> Sterile:: preparePrediction(RooListProxy* _pulls, bool Iosc) const
{
  ...
  for(Int_t i=0;i<33;i++){
    ...
    TFile* outputFile3 = new TFile("outputFigs3.root","RECREATE");// NEW######
    TH1D * forFittingFunction = new TH1D("forFittingFunction","title",30,0.5,8);
    ...
    */
    ...
    TH1D * forOscFittingFunction_100 = new TH1D("forOscFittingFunction_100","title",30,0.5,8);
    ...
    for(Int_t i=1;i<2501;i++)
    {
          ...
          //std::cout<<" outputFig4.root "<<predNEOS->GetBinContent(v)<<std::endl;
    ...
    for(Int_t i=0; i<30; i++)
    {
          std::cout<<" outputFig4.root "<<predNEOS->GetBinContent(i)<<std::endl;
    }
    */
