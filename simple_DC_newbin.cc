...
*/
  dataDC = new TH1D("","dataDC",newBin, 0.5, 8.);
  dataDYB = new TH1D("","dataDYB",newBin, 0.5, 8.);
  dataRENO = new TH1D("","dataRENO",newBin, 0.5, 8.);
  dataNEOS = new TH1D("","dataNEOS",newBin, 0.5, 8.);
  dataPROS = new TH1D("","dataPROS",newBin, 0.5, 8.);
...
TMatrixD* Sterile::prepareCovMatrix(Int_t nBins, TVectorD* fVec) const
{
  ...
  TMatrixD* outMat = new TMatrixD(5 * newBin , 5 * newBin);
  ...
  if(inSyst){
      ...
      fracMatDYB->ResizeTo(newBin,newBin);
      ...
      fracMatRENO->ResizeTo(newBin,newBin);
      ...
      fracMatNEOS->ResizeTo(newBin,newBin);
      ...
      fracMatPROS->ResizeTo(newBin,newBin);
...
//==========================================================================================================================================================2.Matrix==================
      ...
      TVectorD* errList = new TVectorD(newBin);
      for(Int_t i = 0; i< newBin; i++)
      {
      ...
      for(Int_t i = newBin; i< 2 * newBin; i++)
      {
        for(Int_t j = newBin ;j< 2 *newBin; j++) 
        {
	    //std::cout<<"in loading second matrix "<<std::endl;
	    //std::cout<<(*fracMatDYB)(0,0)<<std::endl;
	    //if((*fracMatDYB)(4,4) > 0.5)
	        (*outMat)(i,j) = (*fracMatDYB)(i-newBin,j-newBin) * (*errList)[i-newBin] * (*errList)[j-newBin]  * (*fVec)[i] * (*fVec)[j];
	    //else
      ...
      for(Int_t i = newBin+newBin; i<newBin*3; i++)
      {
        for(Int_t j = newBin+newBin ;j < newBin*3; j++) 
        {
	    //if((*fracMatRENO)(4,4) > 0.5)
	        (*outMat)(i,j) = (*fracMatRENO)(i-newBin-newBin,j-newBin-newBin) * (*errList)[i-newBin-newBin] * (*errList)[j-newBin-newBin]  * (*fVec)[i] * (*fVec)[j];		
	    //else
       	    //    (*outMat)(i,j) = (*fracMatRENO)(i-nBins-nBins,j-nBins-nBins) * (*fVec)[i] * (*fVec)[j];
        }
      }

      for(Int_t i = newBin*3; i< newBin*4; i++)
      {
        for(Int_t j = newBin*3 ;j< newBin*4; j++) 
        {
	    //if((*fracMatNEOS)(4,4) > 0.5)
                (*outMat)(i,j) = (*fracMatNEOS)(i-newBin-newBin-newBin,j-newBin-newBin-newBin) * (*errList)[i-newBin-newBin-newBin] * (*errList)[j-newBin-newBin-newBin]  * (*fVec)[i] * (*fVec)[j];
            //else	    
      ...
      for(Int_t i = newBin*4; i< newBin*5; i++)
      {
        for(Int_t j = newBin*4 ;j< newBin*5; j++)
        {
	    //std::cout<<(*fracMatPROS)(i-4*nBins,j-4*nBins)<<" "<<(*errList)[i-4*nBins]<<" "<<(*fVec)[i]<<" "<<(*fVec)[j]<<std::endl;
            (*outMat)(i,j) = (*fracMatPROS)(i-4*newBin,j-4*newBin) * (*errList)[i-4*newBin] * (*errList)[j-4*newBin]  * (*fVec)[i] * (*fVec)[j];
        }
      }
  }
  //----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  for(Int_t i = 0; i< newBin*5 ; i++)
  {
    (*outMat)(i,i) += (*fVec)[i] ;
...
Double_t Sterile ::FillEv( RooListProxy* _pulls ) const 
{
   ...
   *///June,3,2020 : Print out Scaling4
   ...
   TVectorD* fVec = new TVectorD(newBin * 5);//Up to date prediction. current prediction
   TVectorD* fData = new TVectorD(newBin * 5);
...
*/
   ...
   for(Int_t i=0;i<newBin;i++){
      ...
      (*fData)[newBin + i]     = 0; //dataDYB->GetBinContent(i+1)  - (*fVec)[nBins + i] * scaling2;
      (*fData)[2 * newBin + i] = 0; //dataRENO->GetBinContent(i+1) - (*fVec)[2 * nBins + i] * scaling3 ;
      (*fData)[3 * newBin + i] = 0; //dataNEOS->GetBinContent(i+1) - (*fVec)[3 * nBins + i] * scaling4;
      (*fData)[4 * newBin + i] = 0;
   }
   ...
   if (singleExp.Contains("DYB")){
     ...
     for(Int_t i=0;i<newBin;i++){
        (*fData)[newBin + i]     = dataDYB->GetBinContent(i+1)  - (*fVec)[newBin + i] * scaling2;
     }
   }
   if (singleExp.Contains("RENO")){
     //std::cout<<"DC7_Adding RENO "<<std::endl;
     for(Int_t i=0;i<newBin;i++){
        (*fData)[2 * newBin + i] = dataRENO->GetBinContent(i+1) - (*fVec)[2 * newBin + i] * scaling3 ;
     }
   }
   ...
   if (singleExp.Contains("NEOS")){
     ...
     for(Int_t i=0;i<newBin;i++){
        (*fData)[3 * newBin + i] = dataNEOS->GetBinContent(i+1) - (*fVec)[3 * newBin + i] * scaling4;
        SumfVec += (*fVec)[3 * newBin + i] * scaling4;//May,20,2020
  
        (*testVec)[i]=(*fVec)[3 * newBin + i] * scaling4; //May,21,2020
   ...
   if (singleExp.Contains("PROS")){
     ...
     for(Int_t i=0;i<newBin;i++){
        (*fData)[4 * newBin + i] = dataPROS->GetBinContent(i+1) - (*fVec)[4 * newBin + i] * scaling5;
     }
   }
   if (singleExp.Contains("ALL")){
     ...
     //std::cout<<"DC10_Adding ALL at one time "<<std::endl;
        ...
        (*fData)[newBin + i]     = TMath::Abs( dataDYB->GetBinContent(i+1)  - (*fVec)[newBin + i] * scaling2 );
	(*fData)[2 * newBin + i] = TMath::Abs( dataRENO->GetBinContent(i+1) - (*fVec)[2 * newBin + i] * scaling3 );
	(*fData)[3 * newBin + i] = TMath::Abs( dataNEOS->GetBinContent(i+1) - (*fVec)[3 * newBin + i] * scaling4 );
        (*fData)[4 * newBin + i] = TMath::Abs( dataPROS->GetBinContent(i+1) - (*fVec)[4 * newBin + i] * scaling5 );
...
std::vector<TH1D*> Sterile:: GetFluxPrediction(RooListProxy* _pulls, bool Iosc)  
{
  ...
  TH1D* predDC = new TH1D("","",newBin,binEdge[0],binEdge[newBin]);
  TH1D* predDYB = new TH1D("","",newBin,binEdge[0],binEdge[newBin]);
  TH1D* predRENO = new TH1D("","",newBin,binEdge[0],binEdge[newBin]);
  TH1D* predNEOS = new TH1D("","",newBin,binEdge[0],binEdge[newBin]);
  TH1D* predPROS = new TH1D("","",newBin,binEdge[0],binEdge[newBin]);
 
  //---------------------------------------------------------------------------------------------------------------------------------------------7.2 making prediction graph frame------ 

  for(Int_t i=0;i<newBin+3;i++){
  ...
  for(Int_t i=0;i<newBin+3;i++){
	...
	//std::cout<< "i : " <<i<< " oscNEOS : "<<oscNEOS<<endl;
      if(equalIso){
        ...
        predNEOS -> SetBinContent( i+1, 1. * IBDXsec->Eval((binEdge[i]+binEdge[i+1])/2.) * rateFactorNEOS * (meuller235->Eval((binEdge[i]+binEdge[i+1])/2.) * fissionFraction[12] *  ((RooAbsReal*)_pulls->at(i+12))->getVal() + meuller238->Eval((binEdge[i]+binEdge[i+1])/2.) * fissionFraction[13] + meuller239->Eval((binEdge[i]+binEdge[i+1])/2.) * fissionFraction[14] + meuller241->Eval((binEdge[i]+binEdge[i+1])/2.) * fissionFraction[15] ));
...
//newBin
fHist = new TH2D("","",newBin,binEdge,newBin,binEdge);
TH2D* cfHist = new TH2D("","",newBin,0.5,0.5+_nBins*0.25/2,newBin,0.5,0.5+newBin*0.25/2);
//TH2D* fHist = new TH2D("","",34,0.5,9,34,0.5,9);

fMatrix = new TMatrixD(newBin, newBin);
TMatrixD* cfMatrix = new TMatrixD(newBin, newBin);
uMatrix = new TMatrixD(newBin, newBin);
unfoldingMatrix = new TMatrixD(newBin, newBin);
