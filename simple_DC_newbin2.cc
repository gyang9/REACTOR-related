...
TMatrixD* Sterile::prepareCovMatrix(Int_t nBins, TVectorD* fVec) const
{
  ...
  if(inSyst){
      ...
      {
          for(Int_t j=0;j<newBin_DCDYB;j++)
          {
...
//==========================================================================================================================================================2.Matrix==================
      ...
      {
        for(Int_t j = newBin_DCDYB ;j< newBin_DCDYB; j++) 
        {
...
*/
   ...
   */
   ...
   if (singleExp.Contains("NEOS")){
     ...
     for(Int_t i=0;i<newBin_NEOS;i++){
        (*fData)[newBin_DCDYB + newBin_DCDYB + newBin_NEOS + i] = dataNEOS->GetBinContent(i+1) - (*fVec)[newBin_DCDYB + newBin_DCDYB + newBin_NEOS + i] * scaling4;
        SumfVec += (*fVec)[newBin_DCDYB + newBin_DCDYB + newBin_NEOS  + i] * scaling4;//May,20,2020
  
        (*testVec)[i]=(*fVec)[newBin_DCDYB + newBin_DCDYB + newBin_NEOS  + i] * scaling4; //May,21,2020
   ...
   if (singleExp.Contains("PROS")){
     ...
     for(Int_t i=0;i<newBin_PROS;i++){
        (*fData)[newBin_DCDYB + newBin_DCDYB + newBin_NEOS + newBin_PROS + i] = dataPROS->GetBinContent(i+1) - (*fVec)[newBin_DCDYB + newBin_DCDYB + newBin_NEOS + newBin_PROS + i] * scaling5;
     }
   }
